...
(async function sportyBet() {
  ...
  try {
    ...
    let username = await driver.findElement(
      By.xpath("//input[@name='phone' and @placeholder='Mobile Number']")
    );
    ...
    let loginBtn = await driver.findElement(
      By.xpath("//button[@name='logIn' and .='Login']")
    );
    ...
    try {
      let closeButton = await driver.findElement(
        By.xpath("//div[@class='m-winning-wrapper']//i[@class='m-icon-close']")
      );
    ...
    for (let lgPost = 1; lgPost <= fLeagues.length; lgPost++) {
      let leagueName = await driver
        .findElement(
          By.xpath(
            `(//div[@class='match-league'])[${lgPost}]/div[@class='league-title']/span[@class='text']`
          )
        )
        .getText();
      console.log(`League ${lgPost}: ${leagueName}`);
      if (!checkSimulated(leagueName)) {
        let doubleChanceButton = await driver.findElement(
          By.xpath(
            `(//div[@class='match-league'])[${lgPost}]//div[contains(text(), 'Double Chance')]`
          )
        );
        ...
        let matchPerLeague = await driver.findElements(
          By.xpath(
            `(//div[@class='match-league'])[${lgPost}]//div[contains(@class,'match-row')]`
          )
        );
        console.log(
          `Found ${matchPerLeague.length} matches in league ${lgPost}`
        );

        for (let mPost = 1; mPost <= matchPerLeague.length; mPost++) {
          try {
            let matchHdc = await driver.findElement(
              By.xpath(
                `(//div[@class='match-league'])[${lgPost}]//div[contains(@class, 'match-row')][${mPost}]//div[@class='m-outcome'][1]`
              )
            );
            ...
            console.log(
              `Error finding match odds for match ${mPost} in league ${lgPost}: ${error}`
            );
            if (error.name !== "NoSuchElementError") {
              throw error;
            }
    ...
    let betslip = await driver.findElement(
      By.css("input[placeholder='min. 10']")
    );
    ...
    async function clickPlaceBetButton() {
      try {
        let placeBetLocator = By.css(
          "div.m-btn-wrapper button.af-button.af-button--primary"
        );
        let placeBet = await driver.wait(
          until.elementLocated(placeBetLocator),
          30000
        );
    ...
    async function clickAcceptChangesButton() {
      try {
        ...
        let acceptChanges = await driver.wait(
          until.elementLocated(acceptChangesLocator),
          10000
        );
    ...
    async function clickConfirmBetButton() {
      try {
        let confirmBetLocator = By.css(
          "div.m-btn-wrapper button.af-button.af-button--primary span[data-cms-key='confirm']"
        );
        let confirmBet = await driver.wait(
          until.elementLocated(confirmBetLocator),
          30000
        );
    ...
    fs.appendFileSync(
      "logfile.txt",
      `${matchClicked} matches clicked on ${timeStamp} staked successfully.\n`
    );
  } finally {
    await driver.quit();
  }
